  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (testStarted && !submitted) {
        e.preventDefault();
        e.returnValue = "You have an active test. Are you sure you want to leave? Your progress will not be saved.";
      }
    };

    const handleVisibilityChange = () => {
      if (document.hidden && testStarted && !submitted && !showTabSwitchWarning) {
    ...
    const handleMouseLeave = (e: MouseEvent) => {
      if (testStarted && !submitted && !showTabSwitchWarning) {
        // Check if mouse is leaving towards the top (likely tab bar)
        if (e.clientY <= 0) {
          setShowTabSwitchWarning(true);
        }
      }
            ...
            <Card className="rounded-2xl shadow-md">
              <CardContent className="p-6">
                ...
                <p className="text-lg text-gray-600 mb-6">
                  Welcome to the Mock Test! You will have 1 hour to complete the test. Click the button below to start.
                </p>
                <Button
      ...
      <div className="min-h-screen bg-gradient-to-b from-purple-50 via-white to-white text-gray-800 scroll-smooth">
        <section className="py-20 px-4">
          <div className="max-w-4xl mx-auto text-center">
            <Card className="rounded-2xl shadow-md">
              <CardContent className="p-6">
                <h2 className="text-2xl font-semibold mb-4">Mock Test Submitted</h2>
    ...
    <div className="min-h-screen bg-gradient-to-b from-purple-50 via-white to-white text-gray-800 scroll-smooth">
      <section className="py-20 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-gray-600 mb-6 text-center">
            Select the correct answer for each question and click Submit Test when finished.
          </p>
          <Card className="rounded-2xl shadow-md">
            <CardContent className="p-6">
                    ...
                    return (
                      <div key={optIndex} className="flex items-center mb-1">
                        <input
                          ...
                          name={q._id}
                          value={optionLabel}
                          onChange={() => handleAnswerChange(q._id, optionLabel)}
                          className="mr-2"
                        />
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
          <Card className="rounded-2xl shadow-md bg-purple-50 max-w-md w-full mx-4">
            <CardContent className="p-6 text-center">
              ...
              <p className="text-gray-600 mb-6">
                You have an active test. If you leave, your progress will not be saved. Are you sure you want to leave?
              </p>
              <div className="flex justify-center space-x-4">
                <Button
                  className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2"
      ...
      {showTabSwitchWarning && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
          <Card className="rounded-2xl shadow-md bg-purple-50 max-w-md w-full mx-4">
            <CardContent className="p-6 text-center">
              <h3 className="text-xl font-semibold mb-4">Warning: Test Active</h3>
              <p className="text-gray-600 mb-6">
                Please do not switch tabs during the test. Your progress will not be saved if you leave. Return to the test to continue.
              </p>
              <div className="flex justify-center space-x-4">
                <Button
                  className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2"
